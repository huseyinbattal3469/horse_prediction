        ...
        try:
            for score_param in scoring:
                ...
                cv_results = cross_validate(model, X_train, y_train, cv=cv,
                                            scoring=score_param, n_jobs=n_jobs)  # , error_score="raise"
...
def base_models_gpt4o(X, y, cv=5, scoring=["accuracy", "precision", "recall", "f1", "roc_auc"], is_classifier=True,
                random_state=40, n_jobs=2):
    """
    :param X:
    :param y:
    ...
    def evaluate_model(model, X, y, cv, scoring, n_jobs):
        results = {}
        for score in scoring:
            cv_results = cross_validate(model, X, y, cv=cv, scoring=score, n_jobs=n_jobs)
            results[score] = abs(round(cv_results['test_score'].mean(), 4))
        return results
    ...
    for name, model in models.items():
        ...
        try:
            ...
            scores = evaluate_model(model, X_train, y_train, cv, scoring, n_jobs)
...
def hyperparameter_optimization(X, y, cv=5, scoring="roc_auc", is_classifier=True, is_grid_search=False,
                                random_state=40, n_jobs=2):
    """
    :param X:
    :param y:
    ...
    if is_classifier:
        models = [
            ...
            ("XGBoost", XGBClassifier(), xboost_params),
    ...
    for name, model, params in models:
        ...
        if is_grid_search:
            s_best = GridSearchCV(model, params, cv=cv, n_jobs=n_jobs, verbose=False).fit(X, y)
            final_model = model.set_params(**s_best.best_params_)
        else:
            s_best = RandomizedSearchCV(model, params, cv=cv, n_jobs=n_jobs, verbose=False).fit(X, y)
            final_model = model.set_params(**s_best.best_params_)

        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=.20, random_state=random_state)
        cv_results = cross_validate(final_model, X_train, y_train, cv=cv, scoring=scoring, n_jobs=n_jobs)
...
def hyperparameter_optimization_gpt4o(X, y, cv=5, scoring="roc_auc", is_classifier=True, is_grid_search=False,
                                      random_state=40, n_jobs=2):
    """
    Perform hyperparameter optimization for a given dataset and set of models.
    ...
    is_classifier : bool, default=True
        Whether to use classification models.
    is_grid_search : bool, default=False
        Whether to use GridSearchCV or RandomizedSearchCV.
    random_state : int, default=40
        Random state for reproducibility.
    n_jobs : int, default=2
        Number of parallel jobs for cross-validation and search.

    Returns:
    dict
        A dictionary of best models with their optimized hyperparameters.
    """
    import warnings
    from sklearn.exceptions import ConvergenceWarning, FitFailedWarning
    ...
    model_params = {
        "GNB": (GaussianNB(), {"var_smoothing": [1e-9, 1e-10]}),
        "BNB": (BernoulliNB(), {"alpha": [1.0, 0.5, 0.1], "binarize": [0.0, 0.5, 0.8]}),
        "LR": (LogisticRegression(), {"max_iter": [1000], "penalty": ['l2', 'l1', 'elasticnet'], "C": [0.1, 1.0, 1.5]}),
        "SGD": (SGDClassifier(), {"alpha": [0.0001, 0.0001, 0.01], "penalty": ['l2', 'l1', 'elasticnet', None],
                                  "learning_rate": ['constant', 'optimal', 'adaptive'],
                                  "max_iter": [1000, 500, 2000, 10000], "loss": ['log']}),
        "CART": (DecisionTreeClassifier(), {"max_depth": range(1, 20), "min_samples_split": range(2, 30)}),
        "RF": (RandomForestClassifier(),
               {"max_depth": [8, 15, None], "max_features": [5, 7, "sqrt"], "min_samples_split": [15, 20],
                "n_estimators": [200, 300]}),
        "GBM": (GradientBoostingClassifier(),
                {'learning_rate': [0.09, 0.1, 0.085, 0.08], 'max_depth': [2, 3, 4], 'max_features': [2, 3, 4, None],
                 'max_leaf_nodes': [2, 3, None], 'n_estimators': [100, 200, 250, 300, 500, 1000]}),
        "HIST": (HistGradientBoostingClassifier(),
                 {"learning_rate": [0.1, 0.01, 0.001], "max_iter": [50, 100, 200, 400],
                  "max_leaf_nodes": [31, 15, 45, 60], "max_depth": [None, 1, 2]}),
        "ADABoost": (AdaBoostClassifier(), {'n_estimators': [2, 3, 5, 6, 7, 9, 10, 11, 12, 15, 18],
                                            'learning_rate': [(0.97 + x / 100) for x in range(0, 20)]}),
        "XGBoost": (XGBClassifier(), {"learning_rate": [0.1, 0.01], "max_depth": [5, 8], "n_estimators": [100, 200],
                                      "colsample_bytree": [0.5, 1]}),
        "LightGBM": (LGBMClassifier(verbose=-1), {"learning_rate": [0.01, 0.05, 0.1], "n_estimators": [300, 500, 700],
                                                  "colsample_bytree": [0.7, 0.5, 1]}),
        "Catboost": (CatBoostClassifier(verbose=False),
                     {'learning_rate': [0.09, 0.1, 0.12, 0.13], "max_depth": [3, 4, 5, 6],
                      "n_estimators": [200, 250, 259, 260, 261]}),
        "MLP": (MLPClassifier(),
                {'hidden_layer_sizes': [(100,), (100, 50), (100, 50, 20)], 'activation': ["relu", "tanh", "logistic"],
                 'alpha': [0.0001, 0.01, 0.1],
                 'solver': ["lbfgs", "sgd", "adam"], 'learning_rate_init': [0.001, 0.01, 0.1], 'max_iter': [1000]})
    }

    if not is_classifier:
        model_params.update({
            "RID": (Ridge(), {"alpha": [0.001, 0.01, 0.1, 1.0, 10.0],
                              "solver": ['auto', 'svd', 'cholesky', 'lsqr', 'sparse_cg', 'sag', 'saga']}),
            "LAS": (Lasso(), {"alpha": [0.001, 0.01, 0.1, 1.0, 10.0], "selection": ["cyclic", "random"]}),
            "ENET": (ElasticNet(), {"alpha": [0.001, 0.01, 0.1, 1.0, 10.0], "l1_ratio": [0, 0.5, 0.9, 0.99],
                                    "selection": ["cyclic", "random"]}),
            "CART": (DecisionTreeRegressor(), {"max_depth": range(1, 20), "min_samples_split": range(2, 30)}),
            "RF": (RandomForestRegressor(),
                   {"max_depth": [8, 15, None], "max_features": [5, 7, "sqrt"], "min_samples_split": [15, 20],
                    "n_estimators": [200, 300]}),
            "GBM": (GradientBoostingRegressor(),
            ...
            "HIST": (HistGradientBoostingRegressor(),
                     {"learning_rate": [0.1, 0.01, 0.001], "max_iter": [50, 100, 200, 400],
                      "max_leaf_nodes": [31, 15, 45, 60], "max_depth": [None, 1, 2]}),
            "Adaboost": (AdaBoostRegressor(), {'n_estimators': [2, 3, 5, 6, 7, 9, 10, 11, 12, 15, 18],
                                               'learning_rate': [(0.97 + x / 100) for x in range(0, 20)]}),
            "XGBoost": (XGBRegressor(), {"learning_rate": [0.1, 0.01], "max_depth": [5, 8], "n_estimators": [100, 200],
                                         "colsample_bytree": [0.5, 1]}),
            "LightGBM": (LGBMRegressor(verbose=-1),
            ...
            "Catboost": (CatBoostRegressor(verbose=False),
                         {'learning_rate': [0.09, 0.1, 0.12, 0.13], "max_depth": [3, 4, 5, 6],
                          "n_estimators": [200, 250, 259, 260, 261]}),
            "MLP": (MLPRegressor(), {'hidden_layer_sizes': [(100,), (100, 50), (100, 50, 20)],
                                     'activation': ["relu", "tanh", "logistic"], 'alpha': [0.0001, 0.01, 0.1],
                                     'solver': ["lbfgs", "sgd", "adam"], 'learning_rate_init': [0.001, 0.01, 0.1],
                                     'max_iter': [1000]})
        })

    def perform_search(model, params, X, y, is_grid_search, cv, n_jobs):
        if is_grid_search:
            search = GridSearchCV(model, params, cv=cv, n_jobs=n_jobs, verbose=False)
        else:
            search = RandomizedSearchCV(model, params, cv=cv, n_jobs=n_jobs, verbose=False)
        search.fit(X, y)
        return search
    ...
    for name, (model, params) in model_params.items():
        ...
        search = perform_search(model, params, X, y, is_grid_search, cv, n_jobs)
        final_model = model.set_params(**search.best_params_)

        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.20, random_state=random_state)
        cv_results = cross_validate(final_model, X_train, y_train, cv=cv, scoring=scoring, n_jobs=n_jobs)
        print(f"{scoring} (After): {round(cv_results['test_score'].mean(), 4)}")
        print(f"{name} best params: {search.best_params_}", end="\n\n")
        best_models[name] = final_model
...
def base_multiclass_models(X, y, cv=5,
    ...
    for name, model in models.items():
        print(f"################## {name} ################## ")
        for score_param in scoring:
            cv_results = cross_validate(model, X, y, cv=cv, scoring=score_param, n_jobs=n_jobs)
            print(f"{score_param}: {round(cv_results['test_score'].mean(), 4)} ({name}) ")

        print()


def hyperparameter_multiclass_optimization(X, y, cv=5, scoring="roc_auc_ovr", is_grid_search=True, n_jobs=2):
    """
    :param X:
    :param y:
    :param cv: int
    ...
    for name, model, params in models:
        ...
        print(f"{scoring} (Before): {round(cv_results['test_score'].mean(), 4)}")
        if is_grid_search:
            s_best = GridSearchCV(model, params, cv=cv, n_jobs=n_jobs, verbose=False, refit=scoring).fit(X, y)
        else:
            s_best = RandomizedSearchCV(model, params, cv=cv, n_jobs=n_jobs, verbose=False, refit=scoring).fit(X, y)
        final_model = model.set_params(**s_best.best_params_)
...
def voting_model(best_models: dict, X, y, cv=5, is_classifier=True, is_multiclass=False, voting_type="soft",
    ...
    if is_classifier:
        ...
        voting = VotingClassifier(estimators=list(best_models.items()), voting=voting_type, n_jobs=n_jobs).fit(X, y)
    else:
        print(f"Voting Model: Regression...")
        voting = VotingRegressor(estimators=list(best_models.items()), weights=voting_type, n_jobs=n_jobs).fit(X, y)
    if is_multiclass:
        cv_results = cross_validate(voting, X, y, cv=cv,
                                    scoring=["accuracy", "precision_macro", "recall_macro", "f1_macro", "roc_auc_ovo",
                                             "roc_auc_ovr"], n_jobs=n_jobs,
        ...
        cv_results = cross_validate(voting, X, y, cv=cv, scoring=["neg_mean_absolute_error", "neg_mean_squared_error",
                                    ...
                                    , n_jobs=n_jobs, error_score="raise" if show_error else np.nan)
